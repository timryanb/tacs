...
class TacsMeshGroup(om.Group):
    def initialize(self):
        ...
        self.options.declare(
            ...
            default=None,
            recordable=False,
            desc="a function that takes the volume coordinates and returns a masking vector for the surface coordinates",
        )
    ...
    def setup(self):
        ...
        self.add_subsystem(
            "fea_mesh", TacsMesh(fea_assembler=fea_assembler, discipline=discipline)
        )
        ...
        # Get number of multiplier nodes on all procs
        nmult = fea_assembler.getNumOwnedMultiplierNodes()

        mask_input = MaskedVariableDescription(
            f"x_{discipline}0", shape=nnodes * 3, tags=["mphys_coordinates"]
        )
        mask_output = MaskedVariableDescription(
            f"x_{discipline}0_masked",
        ...
        self.add_subsystem(
            "masker",
            masker,
            promotes_outputs=[(f"x_{discipline}0_masked", f"x_{discipline}0")],
        )
        self.connect(f"fea_mesh.x_{discipline}0", f"masker.x_{discipline}0")

        self.x_a0 = None

        # Check if surface mapper function was defined
        # If so add a masker component for surface nodes
        if surface_mapper is not None:
            ...
            self.x_a0 = x_orig[pts_mask]

            mask_input = MaskedVariableDescription(
                f"x_{discipline}0", shape=x_orig.size, tags=["mphys_coordinates"]
            )
            mask_output = MaskedVariableDescription(
                f"x_{discipline}_surface0",
                shape=pts_mask.size,
                tags=["mphys_coordinates"],
            )
            ...
            # promote BOTH the volume and surface coordinates to the top level
            # this also implicitly connects the volume coordinates to the masker
            self.promotes(
                "masker_surface", inputs=[mask_input.name], outputs=[mask_output.name]
            )
...
class TacsPrecouplingGroup(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem(
            "unmasker",
            unmasker,
            promotes_inputs=[(f"x_{discipline}0_masked", f"x_{discipline}0")],
        )
...
class TacsSolver(om.ImplicitComponent):
    ...
    def setup(self):
        ...
        if self.discipline == "thermal":
            ...
            raise ValueError(
                f"Unknown discipline {self.discipline}. options are 'struct' or 'thermal'"
            )
    ...
    def compute_jacvec_product(self, inputs, d_inputs, d_outputs, mode):
        ...
        if mode == "rev":
            ...
            for func_name in d_outputs:
                ...
                if f"x_{self.discipline}0" in d_inputs:
                    self.sp.addXptSens(
                        [func_name], [d_inputs[f"x_{self.discipline}0"]], scale=d_func
                    )
    ...
    def setup(self):
        ...
        if self.discipline == "thermal":
            ...
            raise NotImplementedError(
                "Only thermal and structural disciplines are supported"
            )
        ...
        if self.coupled:
            ...
            if self.options["surface_mapper"] is not None:
                # get the mapping vector such that xpts_surf = xpts_vol[mapper]
                ...
                masked_in_desc = MaskedVariableDescription(
                    f"{self.rhs_name}_surface",
                    shape=num_pts_surf,
                    tags=["mphys_coupling"],
                )
                unmasked_out_desc = MaskedVariableDescription(
                    self.rhs_name, shape=num_pts_vol, tags=["mphys_coupling"]
                )

                self.add_subsystem(
                    "q_conduct_surf2vol",
                    UnmaskedConverter(
                        input=masked_in_desc,
                        output=unmasked_out_desc,
                ...
                self.connect(
                    f"{self.rhs_name}_surf2vol.{self.rhs_name}",
                    f"solver.{self.rhs_name}",
                )

                # promote the surface heat fluxes to the top level
                self.promotes(
                    "q_conduct_surf2vol", inputs=[(masked_in_desc.name, self.rhs_name)]
                )
        ...
        if self.options["surface_mapper"] is not None:
            ...
            unmasked_in_desc = MaskedVariableDescription(
                self.states_name, shape=num_pts_vol, tags=["mphys_coupling"]
            )
            ...
            self.connect(
                f"solver.{self.states_name}",
                f"{self.states_name}_vol2surf.{unmasked_in_desc.name}",
            )

            # promote the surface temperatures to the top level
            self.promotes(
                f"{self.states_name}_vol2surf",
                outputs=[(f"{masked_out_desc.name}", self.states_name)],
            )
...
class TacsFuncsGroup(om.Group):
    ...
    def setup(self):
        ...
        if scenario_name is None:
            ...
            name = scenario_name
        sp = self.fea_assembler.createStaticProblem(
            name=name, options={"outputDir": "./output"}
        )
